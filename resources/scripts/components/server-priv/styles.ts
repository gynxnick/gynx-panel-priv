 ...
 *
 * Scoped to descendants of `.gynx-server-priv` so it doesn't leak into the
 * legacy admin / dashboard layouts. Mounted from <ServerShell/> via the
 * GynxServerStyles styled-component below.
 *
 * Tokens kept in this file (not pushed to tailwind.config.js) so future
 * tweaks live next to the markup that uses them. Wireframe is high-fidelity:
 * hex values + spacing should match the reference pixel-close.
 */
